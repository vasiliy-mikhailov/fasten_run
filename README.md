...
This installs the `fasten-run` CLI, the Python package, and the local test dependencies.

In `zsh`, keep the extras spec quoted:
...
The project started from a very concrete failure.

I once launched GRPO training for the 2048 game and came back six days later to discover that training had silently stopped being useful: one of the algorithms generated by the LLM had consumed all available memory and corrupted the Python kernel of the Jupyter notebook that was driving the run.

That is the kind of failure FastenRun is meant to prevent.
...
FastenRun also ships a small CLI for one-off runs and CI smoke checks.
...
```json
{"timed_out": false, "exit_code": 0, "stdout": "4\n", "stderr": ""}
```

More CLI examples live in [`examples/cli_examples.md`](examples/cli_examples.md).

---

## End-to-end tests

FastenRun includes real Docker-backed integration tests so you do not have to validate Python, Rust, and pytest/coverage flows by hand after every Docker or image update.
...
Run end-to-end tests against a local Docker daemon:

```bash
pytest -m integration -q
```

### Docker prerequisites for integration tests

The integration suite expects all of the following to be true:

- Docker CLI is installed and on `PATH`
- Python Docker SDK is importable
- Docker daemon is reachable from the current shell
- `docker ps` works

If these prerequisites are not met, `pytest -m integration -q` now **fails with a diagnostic report** instead of silently skipping.

The integration suite pulls and runs real containers for:
- Python `print(2 + 2)`
- Rust compile-and-run
- `pytest` + coverage JSON through the CLI
